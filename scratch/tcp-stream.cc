...
//
 if (1) {
  ...
  /* Create Nodes */
  // and split into 3 categories: apNode, severNode, and sta
  NodeContainer networkNodes;
  networkNodes.Create (numberOfClients + 2);
  //nodes=server + accesspoint + number client= number of client +2 weafre
  //server and accesspoint will be networknods, client will be station node
  ...
  for (NodeContainer::Iterator i = networkNodes.Begin () + 2; i != networkNodes.End (); ++i)
    {
      staContainer.Add (*i);//append a node to sta container
    }
  ...
  wanIpDevices = p2p.Install (serverNode, apNode);//setting link between server node andapnod =p2p properties
  ...
  stack.Install (networkNodes);// internet stack for all nodes
  ...
  Ipv4InterfaceContainer wanInterface = address.Assign (wanIpDevices); //for AP and Server nodes weafre
  Address serverAddress = Address(wanInterface.GetAddress (0));

  /* IPs for WLAN (STAs and AP) */
  address.SetBase ("192.168.1.0", "255.255.255.0");
  address.Assign (wlanDevices);// setting ip for sta and ap weafre
...
// for creating moving device, sta weafre
