...
The images above show the selected representation index for each segment for testing video at episode 1 and episode 170. after 50 episodes, the agent is able to avoid request heavy representation in the beginning which cause buffer stalling. Also when the throughput drop(segment 91,127,181-190), the agent at ep 170 selects low quality level (0,1) to avoid stalling.
